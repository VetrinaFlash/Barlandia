...
## Sprint 5 (game design) — vestiario, livello sopra l'avatar, postazioni di lavoro

Terzo giro di richieste dirette ("cerca dei personaggi dove possiamo cambiare vestiario, mostra il livello sopra, crea dei posti di lavoro").

- **Vestiario avatar** (voce 56 di `docs/GAME-DESIGN.md`, prima solo colore): 4 outfit (maglia/gilet/papillon/grembiule) disegnati proceduralmente come varianti del petto in `avatar.ts`, riusando la colonna `top` di `avatar_config` già presente nello schema dalla migration iniziale ma mai scritta finora. **Compromesso dichiarato**: righe `avatar_config` create prima di questa feature hanno `top = 'tshirt'` (il vecchio default di colonna) — `normalizeOutfit()` in `constants.ts` fa ricadere qualunque valore non riconosciuto sulla maglia base invece di rompere il rendering. Il claim `top` nel token (sessione + rt-token) è **opzionale** apposta: un cookie di sessione firmato prima di questo deploy non ha quel campo, e renderlo obbligatorio avrebbe sloggato tutti gli utenti già connessi al primo deploy.
